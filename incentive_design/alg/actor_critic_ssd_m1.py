"""Actor-critic agents with parameter sharing, used for SSD environment.
...
class ActorCritic(object):
    ...
    def __init__(self, agent_id, agent_name, config, dim_action,
                 dim_obs_tensor, n_agents, nn, n_action_types):
        """Initialization.
    ...
    def create_networks(self):
        ...
        with tf.variable_scope(self.agent_name):
            ...
            self.action_samples_prime = tf.multinomial(
                self.log_probs_prime, 1)
    ...
    def run_actor(self, obs_tensor, sess, epsilon, prime=False):
    ...
    def create_policy_gradient_op(self):
        ...
        """
        # predefined environment reward
        self.r_env = tf.placeholder(tf.float32, [None], 'r_env')
        # 1-hot indicator for action that was taken:
        # clean, collect apple, else
        self.action_type = tf.placeholder(
            tf.float32, [None, self.n_action_types], 'action_type')

        # incentive_function output is [t, n_action_types]
        # Duplicate by number of agents and reshape to get
        # [t groups of n_agents, n_action_types]
        incentive_duplicate = tf.reshape(
            tf.tile(self.designer.incentive_function, [1, self.n_agents]),
            [-1, self.n_action_types])
        r_total = self.r_env # [t groups of n]
        r_total += self.designer.r_multiplier * tf.reduce_sum(tf.multiply(
            incentive_duplicate, self.action_type), axis=1)
        ...
        self.action_taken = tf.placeholder(tf.float32, [None, self.dim_action],
                                           'action_taken')
    ...
    def create_update_op(self):
        ...
        # that includes modification due to ID
    ...
    def reshape_batch(self, buf):
        ...
        # buf.r_env is a list over time of np.array of agents'
        # environment reward, which does not include incentives
        r_env = np.vstack(buf.r_env)
        r_env.shape = (n_steps * self.n_agents)
        ...
        return (obs_tensor, action_1hot, reward, r_env, obs_tensor_next, done)
    ...
    def update(self, sess, buf, epsilon):
        ...
        (obs_tensor, action_1hot, reward, _,
...
class PolicyNewCNN(object):
    ...
    def __init__(self, params, dim_obs, l_action, agent_name):
        self.obs = tf.placeholder(
            tf.float32, [None, dim_obs[0], dim_obs[1], dim_obs[2]],
            'obs_new')
