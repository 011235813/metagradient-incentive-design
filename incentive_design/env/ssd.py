...
from incentive_design.env import maps
from social_dilemmas.constants import CLEANUP_MAP
from social_dilemmas.envs.cleanup import CleanupEnvWithPlanner
...
class Env(object):
    ...
    def __init__(self, config_env):
        ...
        self.env = CleanupEnvWithPlanner(
            ascii_map=ascii_map,
            ...
            planner_ref_point=self.config.planner_ref_point)

        # length of action input to learned reward function
        if self.config.obs_cleaned_1hot:
            self.dim_action_for_incentive = 2
        else:
            self.dim_action_for_incentive = self.l_action
        
        self.steps = 0

    def process_obs(self, obs_dict):
        """
        obs_dict has keys 'agent-0',...,'agent-(n-1)','p'
        """
        new_dict = {}
        for k, v in obs_dict.items():
            if 'agent' in k:
                new_dict[k[-1]] = v/256.0  # extract the agent number
            else:
                new_dict[k] = v/256.0

        return new_dict

    def process_rewards(self, rewards_dict):
        """
        rewards_dict has keys 'agent-0',...,'agent-(n-1)'
        """
        new_dict = {k[-1]: v for k, v in rewards_dict.items()}
        total_agent_reward = sum(new_dict.values())
        # planner's reward is sum of agents' rewards
        new_dict['p'] = total_agent_reward
